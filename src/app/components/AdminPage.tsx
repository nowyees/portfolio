                                                            ...
                                                            {(editingProject.media?.length ? editingProject.media : editingProject.image ? [{ url: editingProject.image, type: 'image' as const, layout: 'full' as const }] : []).map((item, idx) => (
                                                                <div
                                                                    ...
                                                                    className={(editingProject.detailLayout || 'padded') === 'gallery'
                                                                        ? 'shrink-0 w-max h-[420px] flex items-center self-start snap-center bg-[#f5f5f5]'
                                                                        : item.layout === 'half'
                                                                ...
                                                                >
                                                                    {item.type === 'video' || isVideoUrl(item.url)
                                                                        ? <video src={item.url} poster={item.thumbnailUrl} controls muted playsInline className={(editingProject.detailLayout || 'padded') === 'gallery' ? 'block w-auto max-w-none h-full object-contain' : 'block w-full h-auto'} />
                                                                        : <img src={item.url} alt="" className={(editingProject.detailLayout || 'padded') === 'gallery' ? 'block w-auto max-w-none h-full object-contain' : 'block w-full h-auto'} />}
                                                                </div>
                                                            ))}
